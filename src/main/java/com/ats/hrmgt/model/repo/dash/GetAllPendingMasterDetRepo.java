...
public interface GetAllPendingMasterDetRepo  extends JpaRepository<GetAllPendingMasterDet,String>{
	...
	@Query(value = "SELECT\n" + 
			...
			"            AND leave_structure_allotment.del_status=1  ) AS lv_struvt_pending,\n" + 
			"        (     SELECT\n" + 
			"            COUNT('')     \n" + 
			"        FROM\n" + 
			"            m_employees,\n" + 
			"            leave_authority     \n" + 
			"        WHERE\n" + 
			"            m_employees.emp_id != leave_authority.emp_id \n" + 
			"            AND leave_authority.del_status=1 ) AS lv_auth_pending", nativeQuery = true)
