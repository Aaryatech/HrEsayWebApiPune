...
public interface GetClaimListRepo extends JpaRepository<GetClaimList, Integer> {
	...
	@Query(value = "select ca_head_id,emp_id, round(claim_amount) as claim_amount from claim_apply_header where month=:month and year=:year "
			+ "and claim_status=3 and is_paid=1 and emp_id in (:empIds) ", nativeQuery = true)
	List<GetClaimList> getClaimListSaparate(@Param("month") int month, @Param("year") int year,
			@Param("empIds") List<Integer> empIds);
