...
public interface RemainingEmpForAllocationRepository extends JpaRepository<RemainingEmpForAllocation, Integer>{
	 ...
	 * + "    WHERE\n" + "        emp.del_status = 1 and emp.location_id=:locId ",
	 * nativeQuery = true)
	 */
	...
	@Query(value = "SELECT\n" + 
			...
			"        t_shift_assign_daily\n" + 
			"        where emp_id=t_shift_assign_daily.emp_id) \n" + 
			"            and emp_id=emp.emp_id group by emp.emp_id),\n" + 
			"        (select\n" + 
			"            current_shiftid \n" + 
			"        from\n" + 
			"            m_employees \n" + 
	...
	List<RemainingEmpForAllocation> checkRemainingEmployeeForProjection(@Param("locId") int locId);
