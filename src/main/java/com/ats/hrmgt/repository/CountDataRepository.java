...
public interface CountDataRepository extends JpaRepository<CountData, Integer>{
	...
	@Query(value="select UUID() as id,\n" + 
			...
			"(select count('') from tbl_attt_daily_daily d, m_employees e where d.att_date = :date and late_mark=1 and e.emp_id=d.emp_id and e.location_id in (:locId)) as latemark,"
			+ "(select\n" + 
			"            count('') \n" + 
			"        from\n" + 
