...
public interface TempFistDayAssignListRepository extends JpaRepository<TempFistDayAssignList, Integer>{
	...
	@Query(value = "select tas.*,emp.first_name,emp.middle_name,emp.surname,emp.emp_code,"
			+ "IFNULL((select shiftname from tbl_shift_timming where id=tas.shift_id),'') as shift_name,\n" + 
