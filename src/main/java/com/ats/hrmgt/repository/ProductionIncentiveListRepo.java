...
public interface ProductionIncentiveListRepo extends JpaRepository<ProductionIncentiveList, Integer> {
	...
	@Query(value = "select  UUID() as id,da.att_date,da.emp_id,1 as hrs,0 as show_hrs,0 as tot_othr,da.att_status from tbl_attt_daily_daily da where month(att_date)=:month and "
			+ "year(att_date)=:year and emp_id in (:empIds) and lv_sumup_id in(:status)", nativeQuery = true)
	List<ProductionIncentiveList> getPerformanceIncentiveList(@Param("month") int month, @Param("year") int year,
			@Param("empIds") List<Integer> empIds, @Param("status") List<Integer> status);
	...
	List<ProductionIncentiveList> getLateMarkDedListSaparate(@Param("month") int month, @Param("year") int year,
			@Param("empIds") List<Integer> empIds);
