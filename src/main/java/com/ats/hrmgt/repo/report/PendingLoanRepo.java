...
public interface PendingLoanRepo extends JpaRepository<PendingLoanReport, Integer> {
	...
	@Query(value = "SELECT \n" + 
			...
			"    loan.del_status=1 AND emp.del_status=1 and emp.location_id=:locId and emp.depart_id in (:deptIds)\n" + 
			"    ORDER BY loan.loan_repay_start DESC",nativeQuery=true)
	List<PendingLoanReport> getEmpPendingLoanDetails(@Param("locId") int locId,@Param("deptIds") List<Integer> deptIds);
