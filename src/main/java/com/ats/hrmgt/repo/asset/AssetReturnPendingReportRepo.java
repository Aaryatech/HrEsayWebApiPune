...
public interface AssetReturnPendingReportRepo extends JpaRepository<EmpWiseAssetsReport, Integer>{
	...
	@Query(value="SELECT\n" + 
			...
			+ "m_assets.asset_status = 2 AND t_asset_trans.is_lost = 0 AND t_asset_trans.asset_trans_status = 2 AND "
			+ "m_assets.loc_id = m_location.loc_id AND t_asset_trans.use_to_date <= CURRENT_DATE", nativeQuery=true)
	List<EmpWiseAssetsReport> getAssetReturnPendingReport();
	...
	@Query(value="SELECT\n" + 
			...
			+ "t_asset_trans.del_status = 1 AND t_asset_trans.asset_id = m_assets.asset_id AND "
			+ "m_assets.asset_status = 2 AND t_asset_trans.is_lost = 0 AND t_asset_trans.asset_trans_status = 2 AND "
			+ "m_assets.loc_id = m_location.loc_id AND t_asset_trans.use_to_date <= CURRENT_DATE AND m_assets.loc_id = :locId", nativeQuery=true)
	List<EmpWiseAssetsReport> getAssetReturnPendingReportByLocId(@Param("locId") int locId);
}
